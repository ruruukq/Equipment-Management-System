...
if (isset($_POST['login'])) {
    $loginInput = $_POST['emailid']; // This can be either EmailId or UserId
    $password = $_POST['password']; // Raw password input

    // Fetch the user's hashed password from the database
    $sql = "SELECT EmailId, Password, UserId, Status FROM tblusers WHERE EmailId = :loginInput OR UserId = :loginInput";
    ...
    if ($result) {
        // If login input is found, check the password
        if (password_verify($password, $result->Password)) {
            // Password is correct, check account status
            if ($result->Status == 1) {
                // Password is correct and account is active
                $_SESSION['login'] = $result->EmailId; // Store email in session
                $_SESSION['stdid'] = $result->UserId; // Store UserId in session
                echo "<script>document.location ='dashboard.php';</script>";
                exit();
            } else {
                // Account is blocked
                $_SESSION['error1'] = "Your account is blocked. Please contact admin.";
        ...
        // Login input is incorrect (neither EmailId nor UserId matches)
        $_SESSION['error'] = "Invalid Email or User ID";
        header("Location: index.php#login");
        exit;
    }
...
<div id="login" style="padding-top: 50px;">
    ...
    <div class="form-group">
        <label>Enter Email ID or User ID</label>
        <input class="form-control" type="text" name="emailid" required autocomplete="off" />
    </div>
    <div class="form-group">
        <label>Password</label>
        <input class="form-control" type="password" name="password" required autocomplete="off" />
        <p class="help-block"><a href="user-forgot-password.php" style="text-decoration: none;">Forgot Password?</a></p>
    </div>
    <button type="submit" name="login" class="btn btn-info" style="background-color: #1B4D3E; color: #fff;">LOGIN</button> | <a href="signup.php" style="text-decoration: none;">Sign Up</a>
</form>
                            </div>
                        </div>
                    </div>
