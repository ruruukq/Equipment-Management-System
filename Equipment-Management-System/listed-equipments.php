...
<body>
    ...
    <div class="container">
        <div class="row pad-botm">
            <div class="col-md-12">
                <h4 class="header-line" style="position: relative; top: 30px;">Equipment Lists</h4>
            </div>
        </div>
        ...
        <div class="search-bar">
            <form id="search-form" method="GET">
                <div class="form-group">
                    <input type="text" id="search-query" class="form-control" placeholder="Search by Equipment Name, Brand, or Category" name="searchQuery">
                </div>
            </form>
        </div>
    ...
    if (totalProducts <= limit) {
        $("#load-more").text('No more Equipments').attr('disabled', true);
    }

    // "Load More" button click event
    $("#load-more").click(function() {
        $.ajax({
            url: 'load_more_equipments.php', // PHP file where products are fetched
            type: 'GET',
            data: {
                offset: offset,
                limit: limit,
            ...
            success: function(response) {
                if (response) {
                    ...
                    if (response.includes("No more equipments") || response.trim() === '') {
                        ...
                        $("#load-more").text('More Equipment'); // Reset to "More Products" if there are more
                    }
